...
def upsample(source_array, target_height, target_width):
    """
    Upsamples a numpy array (H, W, C) using cv2.resize.
    """
...
def estimate_gradient_bandit(image, prior, args, hook_config, workdir, dynamic_weights):
    ...
    exploration = args.prior_exploration
    fd_eta = args.fd_eta
    prior_lr = args.prior_lr
    
    prior_shape = (prior_size, prior_size, c)
    
    # Generate exploration noise in the prior space
    exp_noise = np.random.randn(*prior_shape)
    
    # Create two query points in the prior space
    q1_prior = prior + exploration * exp_noise
    q2_prior = prior - exploration * exp_noise
    
    # Upsample the prior-space queries to the full image dimension
    q1_full = upsample(q1_prior, h, w)
    q2_full = upsample(q2_prior, h, w)
    
    # Create the two mutated images for evaluation
    mutant1 = image + fd_eta * q1_full
    mutant2 = image + fd_eta * q2_full
    
    mutant1 = np.clip(mutant1, 0, 255)
    mutant2 = np.clip(mutant2, 0, 255)
    ...
    path1 = os.path.join(workdir, fname1)
    path2 = os.path.join(workdir, fname2)

    mutations_data_for_writing = [
        (fname1, encoded1.tobytes()),
        (fname2, encoded2.tobytes())
    ]
    tasks = [
        (path1, hook_config, dynamic_weights, args),
        (path2, hook_config, dynamic_weights, args)
    ]

    try:
        print(f"--- Writing 2 images to host temporary directory for Bandit evaluation ---")
        write_multiple_files_to_host(mutations_data_for_writing, workdir)

        print(f"--- Evaluating 2 mutations with {args.workers} workers ---")
        losses = np.zeros(2)
        ...
        cleanup_pattern = os.path.join(workdir, f"temp_bandit_{run_id}_*.png")
    ...
    # Estimate the gradient in the prior space
    est_grad_prior = est_deriv * exp_noise
    
    # Update the prior
    # Paper uses sign for L-inf and regular update for L2. Assuming L-inf for stability.
    # We use gradient descent, so we subtract.
    updated_prior = prior - prior_lr * np.sign(est_grad_prior)
    
    # Get the full-dimensional gradient by upsampling the updated prior
    gradient_full = upsample(updated_prior, h, w)

    # L2 Normalize the gradient to prevent large steps
...
def main(args):
    ...
    workdir = tempfile.mkdtemp(prefix="bandit_host_attack_", dir=temp_dir_base)
    if temp_dir_base:
    ...
    try:
        ...
        attack_image_for_bandit = attack_image.copy()

        # Initialize prior for Bandit attack
        prior_shape = (args.prior_size, args.prior_size, 3) # H, W, C
        prior = np.zeros(prior_shape, dtype=np.float32)

        m = np.zeros_like(attack_image_for_bandit, dtype=np.float32)
        v = np.zeros_like(attack_image_for_bandit, dtype=np.float32)
        ...
        for i in range(args.iterations):
            ...
            dynamic_weights = {addr: state["dynamic_weight"] for addr, state in hooks_attack_state.items()}
            grad, prior = estimate_gradient_bandit(attack_image_for_bandit, prior, args, hook_config, workdir, dynamic_weights)
            total_queries += 2
            ...
            attack_image_for_bandit -= update_step

            perturbation = np.clip(attack_image_for_bandit - original_image_float, -args.l_inf_norm, args.l_inf_norm)
            attack_image_for_bandit = np.clip(original_image_float + perturbation, 0, 255)
            attack_image = attack_image_for_bandit
            ...
            latest_image_path = os.path.join(args.output_dir, "latest_attack_image_bandit_host.png")
            cv2.imwrite(latest_image_path, attack_image_uint8_bgr)

            if loss < best_loss_so_far:
                best_loss_so_far = loss
                print(f"New best loss found: {loss:.6f}. Saving best image.")
                best_image_path = os.path.join(args.output_dir, "best_attack_image_bandit_host.png")
            ...
            if is_successful:
                ...
                successful_image_path = os.path.join(args.output_dir, "successful_attack_image_bandit_host.png")
        ...
        if attack_image is not None:
            ...
            interrupted_image_path = os.path.join(args.output_dir, "interrupted_attack_image_bandit_host.png")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="A grey-box adversarial attack using the Bandit algorithm (Targetless Host Version).")
    ...
    parser.add_argument("--output-dir", type=str, default="attack_outputs_bandit_host", help="Directory to save output images and logs.")
