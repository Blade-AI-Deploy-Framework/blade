...
#  (This class is unchanged from the SPSA version)
# ==============================================================================
...
class AttackWorker:
    ...
    def initialize(self, process_name):
        ...
        try:
            ...
            template_path = os.path.join(PROJECT_ROOT, 'src', 'attackers', 'gdb_attach_template.py')
    ...
    def evaluate_image(self, image_np, args, dynamic_weights):
        ...
        # Dynamically wait for GDB output to stabilize.
        # This is crucial to avoid a race condition where Frida signals completion
        # before all GDB hook outputs have been received by the host.
        gdb_wait_start_time = time.time()
        while True:
            with self.lock:
                last_gdb_output_count = len(self.gdb_output)

            # Wait for a short interval
            time.sleep(0.2) # 200ms check interval

            with self.lock:
                current_gdb_output_count = len(self.gdb_output)

            if current_gdb_output_count == last_gdb_output_count:
                # If output count has not changed for the interval, assume it's complete.
                break
            
            if time.time() - gdb_wait_start_time > 5: # 5 second overall timeout
                print(f"[Worker-{self.worker_id}] Warning: GDB output stabilization wait timed out after 5s.")
                break
...
# ==============================================================================
#  Parallel Gradient Estimation (NES Version)
# ==============================================================================

def _evaluate_nes_sample_on_worker(task_args):
    """
    Function executed by each thread to evaluate one NES mutation.
    It calls the worker's evaluate_image method and returns only the loss.
    """
    worker, image_mutation, args, dynamic_weights = task_args
    loss, _, _, _ = worker.evaluate_image(image_mutation, args, dynamic_weights)
    return loss

def estimate_gradient_nes_parallel(workers, image, args, dynamic_weights, current_sigma):
    """
    Estimates the gradient using NES by distributing evaluation tasks across multiple workers.
    """
    ...
    if args.attack_y_channel_only:
        pert_shape = (image_shape[0], image_shape[1], 1)
        for _ in range(half_pop_size):
            noise_vectors.append(np.random.randn(*pert_shape))
    else:
        for _ in range(half_pop_size):
            noise_vectors.append(np.random.randn(*image_shape))

    # Create mutation tasks
    tasks = []
    for i, noise in enumerate(noise_vectors):
        if args.attack_y_channel_only:
            # Apply perturbation to Y channel only for both positive and negative mutations
            yuv_image = cv2.cvtColor(image.astype(np.uint8), cv2.COLOR_RGB2YUV).astype(np.float32)
            y_channel = yuv_image[:, :, 0]
            
            y_pos = y_channel + sigma * np.squeeze(noise, axis=-1)
            yuv_pos = yuv_image.copy()
            yuv_pos[:, :, 0] = y_pos
            mutant_pos = cv2.cvtColor(np.clip(yuv_pos, 0, 255).astype(np.uint8), cv2.COLOR_YUV2RGB).astype(np.float32)

            y_neg = y_channel - sigma * np.squeeze(noise, axis=-1)
        ...
        worker_pos = workers[(2 * i) % len(workers)]
        worker_neg = workers[(2 * i + 1) % len(workers)]
        tasks.append((worker_pos, mutant_pos, args, dynamic_weights))
        tasks.append((worker_neg, mutant_neg, args, dynamic_weights))
    
    print(f"--- Estimating gradient with {pop_size} NES samples across {len(workers)} workers ---")
    
    losses = np.zeros(pop_size)
    with ThreadPoolExecutor(max_workers=len(workers)) as executor:
        results = executor.map(_evaluate_nes_sample_on_worker, tasks)
        for i, loss in enumerate(results):
            losses[i] = loss if not np.isinf(loss) else 1e9 # Replace inf with a large number
    ...
    if args.attack_y_channel_only:
        gradient = np.zeros((image.shape[0], image.shape[1], 1), dtype=np.float32)
    ...
    # The original implementation had an issue where lr decay was cancelled out.
    # Correctly normalize the gradient estimate based on the NES formula.
    # The gradient is the average of (loss_diff * noise_vector) scaled by 1 / sigma.
    # The sum is over half_pop_size pairs, and we average over pop_size total samples.
    # The effective formula becomes: grad = sum(diff*noise) / (pop_size * sigma)
    gradient /= (pop_size * sigma)

    return gradient, shaped_losses, noise_vectors
...
def main(args):
    ...
    attack_succeeded_globally = False
    ...
    try:
        ...
        print("\n[+] All workers are ready. Starting NES attack.")
    ...
    total_queries = 0
    try:
        # --- Logging, Strategy, and Image Setup ---
        ...
        log_header = ["restart_num", "iteration", "total_queries", "loss", "best_loss_this_restart", "overall_best_loss", "lr", "num_satisfied_hooks", "total_hooks", "attack_mode", "focus_targets", "hook_details", "iter_time_s", "total_time_s", "success"]
        ...
        for restart_num in range(args.num_restarts):
            if attack_succeeded_globally:
                break
            print(f"\n=================================================")
            print(f"  Starting Restart {restart_num + 1}/{args.num_restarts}")
            print(f"=================================================\n")

            if restart_num == 0:
                attack_image = original_image.copy().astype(np.float32)
                print("--- Restart 1: Using the original image as the starting point. ---")
            else:
                print(f"--- Restart {restart_num + 1}: Generating a new random starting point within L-inf ball. ---")
                noise = np.random.uniform(-args.l_inf_norm, args.l_inf_norm, original_image_float.shape)
                attack_image = np.clip(original_image_float + noise, 0, 255).astype(np.float32)
            
            # Adam optimizer state should be reset for each restart
            adam_step_counter = 0
            if args.attack_y_channel_only:
            ...
            print("\n--- Calculating initial loss for current starting image ---")
            ...
            total_queries += 1
            print(f"Initial loss for this restart: {initial_loss:.6f}")
            ...
            if is_successful_initial:
                print("\nAttack successful on the initial image for this restart!")
                successful_image_path = os.path.join(args.output_dir, "successful_attack_image_nes_parallel.png")
                cv2.imwrite(successful_image_path, cv2.cvtColor(attack_image.astype(np.uint8), cv2.COLOR_RGB2BGR))
                print(f"Successful image saved to: {successful_image_path}")
                attack_succeeded_globally = True
                continue # Go to the next restart, which will be stopped by the check at the top
            
            best_loss_so_far_this_restart = float('inf')
            ...
            for i in range(args.iterations):
                ...
                print(f"\n--- [Restart {restart_num+1}] Iteration {i+1}/{args.iterations} (Total Queries: {total_queries}) ---")
                ...
                current_sigma = args.sigma # Keep sigma constant or implement a separate decay logic
                
                if decay_reason:
                     print(f"Decay triggered by {decay_reason}. New LR: {current_lr:.6f}")

                print(f"Current LR: {current_lr:.6f}, Current NES sigma: {current_sigma:.6f}")

                dynamic_weights = {addr: state["dynamic_weight"] for addr, state in hooks_attack_state.items()}
                grad_raw, shaped_losses, noise_vectors = estimate_gradient_nes_parallel(workers, attack_image, args, dynamic_weights, current_sigma)
                ...
                # The gradient is now correctly scaled, so we use the learning rate directly.
                ...
                # --- Logging, Saving, and Success Check ---
                num_satisfied_hooks = sum(1 for d in hook_diagnostics.values() if d.get("is_satisfied", False))
                total_hooks = len(hook_config)
                all_hooks_satisfied = (num_satisfied_hooks == total_hooks and total_hooks > 0)
                was_successful_this_iter = is_successful or all_hooks_satisfied
                ...
                log_data = [
                    restart_num + 1, i + 1, total_queries, f"{loss:.6f}", f"{best_loss_so_far_this_restart:.6f}", f"{overall_best_loss:.6f}", f"{current_lr:.6f}",
                    num_satisfied_hooks, total_hooks, attack_mode, f'"{targets_str}"', f'"{hook_details_str}"', f"{iter_time:.2f}", f"{total_time_so_far:.2f}",
                    was_successful_this_iter
                ]
                detailed_log_file.write(",".join(map(str, log_data)) + "\n")
                detailed_log_file.flush()

                latest_image_path = os.path.join(args.output_dir, "latest_attack_image_nes_parallel.png")
                cv2.imwrite(latest_image_path, cv2.cvtColor(attack_image_uint8, cv2.COLOR_RGB2BGR))

                if loss < best_loss_so_far_this_restart:
                    best_loss_so_far_this_restart = loss
                    print(f"New best loss for this restart found: {loss:.6f}.")
                    
                    if loss < overall_best_loss:
                        overall_best_loss = loss
                        print(f"!!! New GLOBAL best loss found: {loss:.6f}. Saving best image. !!!")
                        best_image_path = os.path.join(args.output_dir, "best_attack_image_nes_parallel.png")
                ...
                if args.enable_dynamic_focus:
                    # Dynamic focus logic remains the same as SPSA version
                ...
                # --- Success Check ---
                if was_successful_this_iter:
                    if is_successful:
                        print("\nAttack successful! (Frida reported success)")
                    else:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Parallel NES Grey-Box Attack on Android via Frida and GDB.")
    ...
    attack_group.add_argument("--output-dir", type=str, default="attack_outputs_nes_parallel", help="Directory to save outputs.")
    ...
    attack_group.add_argument("--loss-sharpness", type=float, default=8.0, help="Sharpness parameter (beta) for the Softplus loss function.")
    
    nes_group = parser.add_argument_group("NES Settings")
    nes_group.add_argument("--population-size", type=int, default=32, help="Population size for NES. Must be even. This will be distributed among workers.")
    nes_group.add_argument("--sigma", type=float, default=0.1, help="Standard deviation for the Gaussian noise in NES.")
    nes_group.add_argument("--enable-fitness-shaping", action="store_true", help="Enable fitness shaping (ranking) to stabilize gradients.")
    ...
    optimizer_group.add_argument("--grad-smoothing-samples", type=int, default=3, help="Number of recent gradients to average for a smoother update. Set to 1 to disable.")
    ...
    misc_group.add_argument("--num-restarts", type=int, default=1, help="Number of random restarts to find a better local minimum. Set to 1 to disable.")
    ...
    if cli_args.population_size % 2 != 0:
        parser.error("--population-size must be an even number.")
    
    if cli_args.population_size % cli_args.num_workers != 0:
        print(f"[Warning] For best performance, --population-size ({cli_args.population_size}) should be a multiple of --num-workers ({cli_args.num_workers}).")
